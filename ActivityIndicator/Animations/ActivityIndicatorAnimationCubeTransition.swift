//
//  ActivityIndicatorAnimationCubeTransition.swift
...
class ActivityIndicatorAnimationCubeTransition: ActivityIndicatorAnimationDelegate {
    ...
    func setUpAnimation(in layer: CALayer?, size: CGSize, color: NSColor) {
        ...
        translateAnimation.values = [
            ...
            CGSize(width: deltaX, height: 0),
            CGSize(width: deltaX, height: deltaY),
            CGSize(width: 0, height: deltaY),
            CGSize(width: 0, height: 0)
        ]
        ...
        // Draw squares
        for i in 0 ..< 2 {
            let square = ActivityIndicatorShape.rectangle.layerWith(size: CGSize(width: squareSize, height: squareSize), color: color)
